...
import type { Commande, CommandeItem, Client } from '@/types/models';

interface Props {
    commande: Commande & { client: Client; items: CommandeItem[] };
}
...
export default function PaiementsCreate({ commande }: Props) {
    ...
    const [montant, setMontant] = useState<number>(commande.montant_total ?? 0);
    ...
    const clientName = `${commande.client.nom}${commande.client.prenom ? ' ' + commande.client.prenom : ''}`;
    const itemsCount = commande.items?.length ?? 0;
    ...
    return (
        <>
            <Head title={`Paiement — Commande #${commande.id}`} />
                        ...
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 dark:text-zinc-400">
                                    Commande{' '}
                                    <span className="font-mono font-semibold text-[#C8962E]">
                                        #{commande.id}
                                    </span>
                                </p>
                            </div>
                            ...
                            <CardContent className="pt-0">
                                ...
                                <div className="mt-4 flex items-center justify-between px-4 py-3 rounded-xl bg-gradient-to-r from-[#C8962E]/5 to-[#E8B84B]/5 border border-[#C8962E]/10">
                                    ...
                                    <span className="text-lg font-black text-[#C8962E]">
                                        {fmtGnf(commande.montant_total ?? 0)}
                                    </span>
                                </div>
                                ...
                                {commande.items && commande.items.length > 0 && (
                                    <div className="mt-4 space-y-2">
                                        {commande.items.map((item) => (
                                            <div
                                                key={item.id}
                                                className="flex items-center justify-between text-xs text-gray-600 dark:text-zinc-400 px-2"
                                            >
                                                ...
                                                <span className="font-medium text-gray-800 dark:text-zinc-200 shrink-0">
                                                    {fmtGnf(item.prix_total)}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <CardContent className="pt-0 space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            />
                                            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm font-bold text-[#C8962E]">
                                                GNF
                                            </span>
                                        </div>
                                        <p className="mt-1.5 text-xs text-gray-400 dark:text-zinc-500">
                                            Total commande : {fmtGnf(commande.montant_total ?? 0)}
                                        </p>
                                    </div>
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${
                                                            isSelected
                                                                ? 'border-[#C8962E] bg-[#C8962E]/5 shadow-sm'
                                                                : 'border-gray-200 dark:border-zinc-700 hover:border-gray-300 dark:hover:border-zinc-600 bg-gray-50 dark:bg-zinc-800/50'
                                                        }`}
                                                    >
                                                        <Icon
                                                            className={`w-5 h-5 ${
                                                        ...
                                                        {isSelected && (
                                                            <CheckCircle className="w-3.5 h-3.5 text-[#C8962E]" />
                                                        )}
                                                    </button>
                                                );
                                            })}
                                    ...
                                    <div className="rounded-xl bg-gradient-to-br from-[#C8962E]/5 to-[#E8B84B]/5 border border-[#C8962E]/10 p-4">
                                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400 mb-3">
                                            Récapitulatif
                                        </p>
                                        <div className="space-y-1.5 text-sm">
                                            ...
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-semibold text-gray-900 dark:text-white">
                                                    {modeOptions.find((m) => m.value === modePaiement)?.label}
                                                </span>
                                            </div>
                                            <div className="flex justify-between">
