...
interface Props { depot: Depot }

export default function DepotsEdit({ depot }: Props) {
    const [form, setForm] = useState({ nom: depot.nom ?? '', adresse: depot.adresse ?? '', description: depot.description ?? '' });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.put(`/dashboard/depots/${depot.id}`, form, { onFinish: () => setProcessing(false) });
    };

    return (
        <>
            <Head title={`Modifier — ${depot.nom}`} />
            ...
            <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 flex items-center gap-4">
                ...
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">Modifier — {depot.nom}</h1>
            </div>
